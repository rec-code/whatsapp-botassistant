...
class BotGoogle(BotModulesCore):
	...
	def __init__(self, name, bot):
		super(BotGoogle, self).__init__(name, bot)
		...
		for cred in root:
			if cred.attrib['name'] == 'google':
				self.googleInfos['apiKey'] = cred[0].text
				self.googleInfos['searchEngineId'] = cred[1].text
				self.printi('Google credentials loaded', 'core')


		self.googleService = build(
			"customsearch", 
			"v1", 
			developerKey=self.googleInfos['apiKey']
		)

		self.retrieve_times_allowed = 3
	...
	def search(self, message, bot):
		...
		if 'items' in res:
			bot.get_message('%s resultados no tio Google para *%s*:' % (temp_number, temp_message))
			...
			bot.get_message('Não achei nenhum resultado na net de *%s*, malz mestre <3' % temp_message)
